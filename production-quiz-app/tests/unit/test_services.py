...
from tests.factories import UserFactory, UnverifiedUserFactory, LockedUserFactory
...
class TestAuthService:
    ...
    def test_verify_email_already_verified(self, app):
        ...
        with app.app_context():
            user = UserFactory(email_verified=True, verification_token='old_token')
            db.session.commit()
    ...
    def test_change_password_nonexistent_user(self, app):
        ...
        with app.app_context():
            ...
            assert success is False
            assert 'not found' in message.lower() or 'user' in message.lower()
